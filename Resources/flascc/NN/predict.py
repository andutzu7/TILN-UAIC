...
from NN.extract_article import extract_article
from NN.pipeline import pipeline

new_model = tf.keras.models.load_model('./NN/model92.h5')

df=pd.read_csv('./NN/corpus_complet.tsv',delimiter='\t')
df = df.dropna()
...
def predict_by_link(link):
    text = extract_article(link)
    predictions = predict_by_text(text)
...
def print_predictions(predictions):
    ...
    for index,prediction in enumerate(predictions):
        ...
        return f'{answer(prediction[0])}, percentage = {prediction[0]}'